...
def process_image(image_path, output_dir, args):
    try:
        ...
        # findContours often returns thousands of tiny noise artifacts. Filtering them
        # out first reduces the expensive O(N log N) sorting step to O(K log K)
        # where K << N.
        valid_contours = [
...
class MainWindow(QMainWindow):
    ...
    def dropEvent(self, event):
        if event.mimeData().hasUrls():
            ...
            existing_items = {
                self.file_list_widget.item(i).text() for i in range(
                    self.file_list_widget.count())}
            for url in urls:
                if url.isLocalFile():
