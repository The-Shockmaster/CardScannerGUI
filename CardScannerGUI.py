...
def auto_orient_card(image):
    try:
        osd = pytesseract.image_to_osd(
            image, output_type=pytesseract.Output.DICT, config='--psm 0')
        rotation = osd['rotate']
        if rotation == 90:
...
def enhance_card(image, clahe_clip, clahe_grid, bilateral_d, bilateral_sigma):
    ...
    final_image = cv2.bilateralFilter(
        final_image, bilateral_d, bilateral_sigma, bilateral_sigma)
    return final_image
...
def process_image(image_path, output_dir, args):
    try:
        ...
        thresh = cv2.adaptiveThreshold(
            blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, args.thresh_block_size, 2)
        ...
        # ⚡ Bolt Optimization: Prevent O(N log N) sorts on noise contours
        # We pre-calculate areas and filter out small noise (O(N)) before sorting.
        # This drastically reduces the number of elements passed to the sort algorithm,
        # especially for noisy images with many tiny artifacts.
        valid_contours = []
        for c in contours:
        ...
        # Sort only the pre-filtered, large contours
        ...
        for i, (area, contour) in enumerate(valid_contours):
            ...
            if len(approx) == 4:
                ...
                warped = perspective_transform(
                    original_image, approx.reshape(4, 2))
                oriented = auto_orient_card(warped)
                resized = cv2.resize(
                    oriented, (args.width, args.height), interpolation=cv2.INTER_AREA)
                enhanced = enhance_card(
                    resized, args.clahe_clip, args.clahe_grid, args.bilateral_d, args.bilateral_sigma)
...
class DragDropWidget(QLabel):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            "border: 2px dashed #aaa; border-radius: 5px; font-size: 16px; color: #888;")

    def dragEnterEvent(self, event): event.acceptProposedAction(
    ) if event.mimeData().hasUrls() else event.ignore()
...
class MainWindow(QMainWindow):
    ...
    def dragEnterEvent(
        self, event): self.drag_drop_widget.dragEnterEvent(event)

    def dropEvent(self, event):
        if event.mimeData().hasUrls():
            urls = event.mimeData().urls()
            existing_items = {self.file_list_widget.item(
    ...
    def start_processing(self):
        ...
        file_paths = [self.file_list_widget.item(
            i).text() for i in range(self.file_list_widget.count())]
    ...
    def on_processing_finished(self):
        ...
        QMessageBox.information(
            self, "Success", "All images have been processed successfully.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        error_msg.setInformativeText(
            "Tesseract is not installed or not in your system's PATH. Please install it and try again.")
