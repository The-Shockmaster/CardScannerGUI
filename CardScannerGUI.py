...
def process_image(image_path, output_dir, args):
    try:
        ...
        # This prevents expensive O(N log N) sorting operations on thousands of noise artifacts
        # and avoids redundant cv2.contourArea calculations.
        valid_contours = [
            c for c in contours if cv2.contourArea(c) >= args.min_area]

        found_cards = 0
        for i, contour in enumerate(
                sorted(valid_contours, key=cv2.contourArea, reverse=True)):
            ...
            for url in urls:
                if url.isLocalFile():
                    ...
                    if file_path.lower().endswith(('.png', '.jpg', '.jpeg')
                                                  ) and file_path not in existing_items:
