...
def process_image(image_path, output_dir, args):
    try:
        ...
        thresh = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                       cv2.THRESH_BINARY_INV, args.thresh_block_size, 2)
        kernel = np.ones((5, 5), np.uint8)
        thresh = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel, iterations=2)
        thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, kernel, iterations=2)
        contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # ⚡ Bolt: Pre-filter contours by area to avoid sorting thousands of noise artifacts
        # This reduces the O(N log N) sorting operation to only act on valid candidates
        ...
        for i, contour in enumerate(sorted(valid_contours, key=cv2.contourArea, reverse=True)):
            ...
            if len(approx) == 4:
                ...
                enhanced = enhance_card(resized, args.clahe_clip, args.clahe_grid,
                                        args.bilateral_d, args.bilateral_sigma)
